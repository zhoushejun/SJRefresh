...
//- (void)layoutSubviews {
//    self.activityIndicatorView.center = CGPointMake(self.bounds.size.width/2-40, self.bounds.size.height/2);
//
//    self.titleLabel.center = CGPointMake(self.bounds.size.width/2+20, self.bounds.size.height/2);
...
//        if (![_titleLabel.text isEqualToString:@"没有更多记录"]) {
//            _titleLabel.text = NSLocalizedString(@"正在加载数据",);
//        }
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake((self.bounds.size.width - 320)/2, roundf((self.bounds.size.height-20)/2), 320, 20)];
        _titleLabel.text = NSLocalizedString(@"正在加载数据",);
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    if(hasCustomView) {
        ...
        CGPoint origin = CGPointMake(roundf((self.bounds.size.width-viewBounds.size.width)/2), roundf((self.bounds.size.height-viewBounds.size.height)/2));
        [self.activityIndicatorView setFrame:CGRectMake(origin.x-60, origin.y, viewBounds.size.width, viewBounds.size.height)];
